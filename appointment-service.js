...
(function (global) {
  ...
  async function loadTransitions() {
    ...
    var res = await sb().schema('iapp')
      .from('appointment_transitions').select('*');
    if (res.error) return (_transitions = []);
    return (_transitions = res.data || []);
  }
  ...
  async function actionsFor(appointment, role) {
    ...
    return all
      .filter(function (t) {
        return t.from_status === appointment.status
            && (t.allowed_roles || []).indexOf(role) !== -1;
      })
      .map(function (t) {
  ...
  function isLive(s) {
    return s === STATUS.ARRIVED || s === STATUS.WAITING || s === STATUS.IN_CLINIC;
  }
  function isOpen(s) {
  ...
  IAPP.appointments = {
    ...
    isLive: isLive, isOpen: isOpen, waitLabel: waitLabel, todayISO: todayISO,
