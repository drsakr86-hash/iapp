...
(function (global) {
  ...
  var _sb = null, _session = null, _profile = null, _lastProfileError = null;
  ...
  async function getProfile(force) {
    ...
    // تشخيص دقيق: أربعة أسباب مختلفة كانت تعطي نفس الرسالة المبهمة
    if (r.error) {
      _lastProfileError = 'تعذّر قراءة ملف الحساب: ' + r.error.message
        + ' — تأكد أن ملف IAPP_FIX_IDENTITY_RLS.sql شُغِّل';
      _profile = null; return null;
    }
    if (!r.data) {
      _lastProfileError = 'لا يوجد ملف تعريف لهذا الحساب في جدول profiles';
      _profile = null; return null;
    }
    if (!r.data.is_active) {
      _lastProfileError = 'الحساب معطَّل (is_active = false)';
      _profile = null; return null;
    }
  ...
  async function signIn(email, password) {
    ...
    if (!p) {
      var why = _lastProfileError || 'سبب غير معروف';
      await signOut();
  ...
  function translateAuthError(msg) {
    ...
    if (/Email not confirmed/i.test(msg))       return 'البريد غير مؤكَّد في Supabase Auth';
  ...
  global.IAPP = Object.assign(global.IAPP || {}, {
    ...
    lastProfileError: function(){ return _lastProfileError; },
    config: { url: SB_URL }
  });
})(window);
