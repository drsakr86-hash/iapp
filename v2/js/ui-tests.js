...
IAPP.svc = {
  ...
  examinations: {
    listByPatient: async()=>([{id:'x1', exam_date:'2026-08-20', va_right:'6/9', va_left:'CF',
      va_right_corrected:'6/6', va_right_ph:'6/12',
    ...
    getFull: async id => ({ id:id, patient_id:'p1', exam_date:'2026-08-20',
      va_right:'6/9', va_left:'CF', va_right_corrected:'6/6', va_right_ph:'6/12',
      anterior_segment:'طبيعي', treatment_plan:'نظارة طبية',
      _iop_od:{value_mmhg:26, method:'Goldmann'}, _iop_os:{value_mmhg:15},
      _ref_od:{sphere:-2.25, cylinder:-0.75, axis:90},
      _map:{ OD:{cornea:'عتامة مركزية', lens:'مياه بيضاء نووية'},
             OS:{cornea:'شفافة'}, OU:{cd_ratio:'0.4'} } }),
    comparePair: async (pid,cur) => { window.__calls.cmp={pid,cur}; return {
      cur:{ id:'x1', exam_date:'2026-08-20', va_right:'6/9', va_left:'CF',
            va_right_ph:'6/12', iop_od:26, sph_od:-2.25,
            _map:{OD:{cornea:'عتامة مركزية'},OS:{},OU:{cd_ratio:'0.6'}} },
      prev:{ id:'x0', exam_date:'2026-02-10', va_right:'6/6', va_left:'CF',
            iop_od:18, sph_od:-2.00,
            _map:{OD:{cornea:'شفافة'},OS:{},OU:{cd_ratio:'0.3'}} },
      all:[ { id:'x1', exam_date:'2026-08-20', va_right:'6/9', va_left:'CF',
              va_right_ph:'6/12', iop_od:26, sph_od:-2.25,
              _map:{OD:{cornea:'عتامة مركزية'},OS:{},OU:{cd_ratio:'0.6'}} },
            { id:'x0', exam_date:'2026-02-10', va_right:'6/6', va_left:'CF',
              iop_od:18, sph_od:-2.00,
              _map:{OD:{cornea:'شفافة'},OS:{},OU:{cd_ratio:'0.3'}} } ] }; }
  },
  surgeries: {
    listByPatient: async()=>([{id:'s1', eye:'OD', procedure_name:'المياه البيضاء بالفاكو + عدسة',
      performed_on:'2025-11-02', surgeon_name:'د. عبده', anesthesia:'موضعي بالقطرة',
      outcome:'ناجحة بلا مضاعفات', is_planned:false, is_external:false}]),
    create: async r => { window.__calls.surg={mode:'create',row:r}; return {id:'s9'}; },
    update: async (id,r) => { window.__calls.surg={mode:'update',id,row:r}; return {id}; },
    remove: async()=>true
  },
  diagnoses: {
...
(async function(){
  ...
  ok('يعرض رقم الإصدار P10', doc.body.textContent.indexOf('P10')>-1);
  ...
  // ستة منذ المرحلة 9 (أُضيف تبويب العمليات)
  ok('تبويبات الملف السبعة (+ الصور)', $$('[data-pt]').length===7,
     $$('[data-pt]').length+'');
  ...
  // المرحلة 9: القياسان يُعرضان — والعرض بلا قفل وعدٌ كاذب لأن الحفظ لا يرسلهما
  ok('🔴 ضغط العين معروض في التعديل لا مخفيّ', !!doc.getElementById('e_iopd'));
  ok('🔴 لكنه مقفل لأن الحفظ لا يرسله', doc.getElementById('e_iopd').disabled===true);
  ok('🔴 الانكسار مقفل كذلك', doc.getElementById('r_od_sph').disabled===true);
  ok('طريقة القياس مقفلة', doc.getElementById('e_iopm').disabled===true);
  ok('الانكسار القديم معروض',
     doc.getElementById('r_od_sph').value==='-2.25', doc.getElementById('r_od_sph').value);
  ok('الموجودات القديمة محمَّلة في الشبكة',
     doc.getElementById('f_cornea_od').value==='عتامة مركزية',
     doc.getElementById('f_cornea_od').value);
  ok('OU تُقرأ كبديل حين لا OD ولا OS',
     doc.getElementById('f_cd_ratio_od').value==='0.4',
     doc.getElementById('f_cd_ratio_od').value);
  click(doc.getElementById('mcancel'));
  ...
  // ── 12. المرحلة 9: السجل السريري المبنيّ ───────────────
  console.log('\x1b[1m12. المرحلة 9 — السجل المبنيّ\x1b[0m');

  const Mm = w.IAPP.models;
  ok('حقول القطاع الأمامي سبعة', Mm.ANT_FIELDS.length===7, ''+Mm.ANT_FIELDS.length);
  ok('حقول القطاع الخلفي ستة',  Mm.POST_FIELDS.length===6, ''+Mm.POST_FIELDS.length);
  ok('كل حقل يعرف قطاعه', Mm.FIELD_SECTION.cornea==='anterior'
     && Mm.FIELD_SECTION.macula==='posterior');
  ok('C/D خارج المدى مرفوضة', !Mm.validateCD('1.4'));
  ok('C/D بصيغة المدى مقبولة', Mm.validateCD('0.3-0.4'));
  ok('عملية أُجريت بلا تاريخ مرفوضة',
     !Mm.validateSurgery({patient_id:'p1',procedure_name:'فاكو',eye:'OD'}).ok);
  ok('عملية مخطَّطة بلا تاريخ مقبولة',
     Mm.validateSurgery({patient_id:'p1',procedure_name:'فاكو',eye:'OD',is_planned:true}).ok);
  ok('الغياب ليس تغيّراً',
     Mm.compareRows({a:'x'},{},[{key:'a',label:'a',get:o=>o.a}])[0].changed===false);
  ok('اختلاف قيمتين موجودتين تغيّر',
     Mm.compareRows({a:'x'},{a:'y'},[{key:'a',label:'a',get:o=>o.a}])[0].changed===true);

  // ── النموذج الكامل ──────────────────────────────────────
  // القسم يفتح مريضه بنفسه: الاتّكال على ما تركه القسم السابق جعل
  // التأكيد يقرأ p2 بينما يظنّ أنه p1 — عطب اختبار لا عطب تطبيق.
  await goDay();
  click(doc.querySelector('[data-open="p1"]'));
  await sleep(180);
  click(byText('[data-pt]','الفحوصات'));
  await sleep(120);
  CALLS.exam=null;
  click(doc.getElementById('newexam'));
  await sleep(140);
  ok('حقول الإبصار الثلاثة موجودة',
     !!doc.getElementById('e_vad') && !!doc.getElementById('e_bcvad')
     && !!doc.getElementById('e_phd'));
  ok('الانكسار للعينين', !!doc.getElementById('r_od_sph') && !!doc.getElementById('r_os_cyl'));
  ok('طريقة الضغط ووقته', !!doc.getElementById('e_iopm') && !!doc.getElementById('e_iopt'));
  ok('شبكة القطاع الأمامي كاملة',
     Mm.ANT_FIELDS.every(f=>!!doc.getElementById('f_'+f[0]+'_od')));
  ok('شبكة القطاع الخلفي كاملة',
     Mm.POST_FIELDS.every(f=>!!doc.getElementById('f_'+f[0]+'_os')));
  ok('الملاحظة الحرّة على القطاعين محفوظة',
     !!doc.getElementById('e_ant') && !!doc.getElementById('e_post'));
  ok('لا datalist في السجل المبنيّ', $$('datalist').length===0);

  // زرّ «الكل طبيعي» يملأ ولا يدهس
  set('f_cornea_od','قرحة');
  click(doc.getElementById('ant_norm'));
  ok('«الكل طبيعي» لا يدهس ما كُتب',
     doc.getElementById('f_cornea_od').value==='قرحة');
  ok('«الكل طبيعي» يملأ الفارغ',
     doc.getElementById('f_lids_od').value==='طبيعية',
     doc.getElementById('f_lids_od').value);
  ok('زرّ قطاع لا يتخطّى حدوده',
     doc.getElementById('f_cd_ratio_od').value==='',
     'C/D خلفية ولا يملؤها زرّ الأمامي');
  click(doc.getElementById('post_norm'));
  ok('C/D الطبيعية 0.3 لا 0.1',
     doc.getElementById('f_cd_ratio_od').value==='0.3',
     doc.getElementById('f_cd_ratio_od').value);

  // النسخ يمين ← يسار
  click(doc.getElementById('ant_copy'));
  ok('نسخ اليمنى لليسرى', doc.getElementById('f_cornea_os').value==='قرحة');

  // OU يطوي عمود اليسرى
  const ouBox = doc.getElementById('post_ou');
  ouBox.checked = true;
  ouBox.dispatchEvent(new w.Event('change',{bubbles:true}));
  await sleep(40);
  ok('OU يطوي عمود العين اليسرى',
     doc.querySelector('.seg[data-sec="post"]').className.indexOf('ou')>-1);
  ok('عنوان العمود يتبدّل إلى OU',
     doc.getElementById('post_odlbl').textContent.indexOf('OU')>-1);

  set('f_disc_od','تجويف');
  set('f_cd_ratio_od','0.7');
  set('e_bcvad','6/6'); set('e_phd','6/12');
  set('r_od_sph','-2.25'); set('r_od_cyl','-0.75'); set('r_od_axis','90');
  set('e_iopd','26'); set('e_iops','15');
  doc.getElementById('e_iopm').value='Goldmann';
  doc.getElementById('e_iopt').value='11:30';

  click(doc.getElementById('save'));
  await sleep(160);
  ok('الفحص أُرسل', !!CALLS.exam);
  ok('BCVA مرفقة', CALLS.exam.va_right_corrected==='6/6');
  ok('الثقب PH مرفق', CALLS.exam.va_right_ph==='6/12');
  ok('الانكسار مرفق',
     CALLS.exam.ref_od_sph==='-2.25' && CALLS.exam.ref_od_axis==='90');
  ok('طريقة الضغط مرفقة', CALLS.exam.iop_method==='Goldmann');
  ok('وقت الضغط تاريخ ووقت كاملان',
     /^\d{4}-\d{2}-\d{2}T11:30:00$/.test(String(CALLS.exam.iop_at)), String(CALLS.exam.iop_at));

  const F = CALLS.exam.findings || [];
  const get = (eye,f) => (F.filter(x=>x.eye===eye&&x.field===f)[0]||{}).value;
  ok('الموجودات مرفقة كصفوف', F.length>0, ''+F.length);
  ok('القرنية اليمنى', get('OD','cornea')==='قرحة');
  ok('القرنية اليسرى', get('OS','cornea')==='قرحة');
  ok('🔴 القطاع الخلفي مع OU يُرسل مرة واحدة بعين OU',
     get('OU','disc')==='تجويف' && get('OD','disc')===undefined
     && get('OS','disc')===undefined);
  ok('C/D مع OU', get('OU','cd_ratio')==='0.7');
  ok('لا حقل فارغ يُرسل', F.every(x=>x.value!=null && x.value!==''));
  ok('كل موجود يحمل حقلاً معروفاً', F.every(x=>!!Mm.FIELD_LABEL[x.field]));

  // ── المقارنة الطولية ────────────────────────────────────
  CALLS.cmp=null;
  click(byText('[data-pt]','الفحوصات'));
  await sleep(110);
  ok('زر المقارنة على بطاقة الفحص', !!$('[data-cmp]'));
  click($('[data-cmp]'));
  await sleep(160);
  ok('فُتحت شاشة المقارنة', !!$('.cmp') || doc.body.textContent.indexOf('مقارنة')>-1);
  ok('الخدمة استُدعيت بالمريض والفحص',
     CALLS.cmp && CALLS.cmp.pid==='p1' && CALLS.cmp.cur==='x1');
  const cmpTxt = $('.cmp') ? $('.cmp').textContent : '';
  ok('تاريخا الزيارتين ظاهران',
     cmpTxt.indexOf('2026-08-20')>-1 && cmpTxt.indexOf('2026-02-10')>-1);
  ok('الضغط المتغيّر معروض', cmpTxt.indexOf('26 mmHg')>-1 && cmpTxt.indexOf('18 mmHg')>-1);
  ok('C/D المتغيّرة معروضة', cmpTxt.indexOf('0.6')>-1 && cmpTxt.indexOf('0.3')>-1);
  ok('🔴 البند غير المتغيّر مخفي افتراضاً',
     cmpTxt.indexOf('CF')===-1, 'حدة اليسرى لم تتغيّر فيجب ألّا تظهر');
  ok('قوة العدسة تحمل إشارتها', cmpTxt.indexOf('-2.25')>-1);
  const allBox = doc.getElementById('c_all');
  allBox.checked = true;
  allBox.dispatchEvent(new w.Event('change',{bubbles:true}));
  await sleep(90);
  ok('«أظهر غير المتغيّر» يكشف الثابت',
     $('.cmp').textContent.indexOf('CF')>-1);
  ok('مبدّل الزيارة موجود', !!doc.getElementById('c_pick'));
  click(doc.getElementById('mcancel'));
  await sleep(40);
  ok('المقارنة تُغلق', !$('.cmp'));

  // ── التاريخ الجراحي ─────────────────────────────────────
  click(byText('[data-pt]','العمليات'));
  await sleep(130);
  ok('تبويب العمليات يعرض السجل',
     doc.body.textContent.indexOf('المياه البيضاء بالفاكو')>-1);
  ok('عين العملية معروضة بالعربي', doc.body.textContent.indexOf('اليمنى')>-1);
  CALLS.surg=null;
  click(doc.getElementById('newsurg'));
  await sleep(120);
  ok('نموذج العملية فُتح', !!doc.getElementById('s_name'));
  click(doc.getElementById('save'));
  await sleep(90);
  ok('🔴 عملية بلا اسم ولا عين لا تُرسل', CALLS.surg===null);
  doc.getElementById('s_eye').value='OS';
  pick('s_name','ليزك LASIK');
  ok('قائمة العمليات تكتب في الحقل',
     doc.getElementById('s_name').value==='ليزك LASIK');
  click(doc.getElementById('save'));
  await sleep(90);
  ok('🔴 عملية أُجريت بلا تاريخ لا تُرسل — القيد نفسه في القاعدة',
     CALLS.surg===null);
  doc.getElementById('s_plan').checked = true;
  click(doc.getElementById('save'));
  await sleep(120);
  ok('مخطَّطة بلا تاريخ تُقبل', CALLS.surg && CALLS.surg.mode==='create');
  ok('العين مرسلة', CALLS.surg.row.eye==='OS');
  ok('العلم مرسل', CALLS.surg.row.is_planned===true);

  CALLS.surg=null;
  click($('[data-esurg]'));
  await sleep(120);
  ok('التعديل يحمّل القيم', doc.getElementById('s_name').value.indexOf('الفاكو')>-1);
  ok('التاريخ محمَّل', doc.getElementById('s_on').value==='2025-11-02');
  doc.getElementById('s_out').value='مضاعفات';
  click(doc.getElementById('save'));
  await sleep(120);
  ok('التعديل أُرسل', CALLS.surg && CALLS.surg.mode==='update');
  ok('النتيجة الجديدة مرسلة', CALLS.surg.row.outcome==='مضاعفات');
