...
(function (global) {
  ...
  var imaging = {
    ...
    // ══════════════════════════════════════════════════════
    // طلبات الأشعة (ترويسة + بنود)
    // ══════════════════════════════════════════════════════
    orders: {

      async listByPatient(pid, limit){
        var r = await sb().from('imaging_orders')
          .select('id,order_no,patient_id,visit_id,doctor_id,clinic_id,ordered_on,'
                + 'urgency,clinical_indication,clinical_notes,status,printed_count,'
                + 'printed_at,created_at')
          .eq('patient_id', pid).is('deleted_at', null)
          .order('ordered_on', {ascending:false})
          .order('created_at', {ascending:false})
          .limit(limit || 25);
        if(r.error) throw err(r.error, 'قراءة الطلبات');
        var rows = r.data || [];
        if(!rows.length) return rows;

        var ids = rows.map(function(x){ return x.id; });
        var it = await sb().from('imaging_order_items')
          .select('id,order_id,seq,modality,eye,notes,image_id,done')
          .in('order_id', ids).order('seq', {ascending:true});
        if(it.error) throw err(it.error, 'قراءة بنود الطلب');

        var by = {};
        (it.data || []).forEach(function(x){
          (by[x.order_id] = by[x.order_id] || []).push(x);
        });
        rows.forEach(function(x){ x.items = by[x.id] || []; });
        return rows;
      },

      async get(id){
        var r = await sb().from('imaging_orders').select('*')
                .eq('id', id).is('deleted_at', null).maybeSingle();
        if(r.error) throw err(r.error, 'قراءة الطلب');
        if(!r.data) throw new Error('الطلب غير موجود');
        var it = await sb().from('imaging_order_items')
                 .select('id,order_id,seq,modality,eye,notes,image_id,done')
                 .eq('order_id', id).order('seq', {ascending:true});
        r.data.items = (it.data || []);
        return r.data;
      },

      /**
       * الترويسة أولاً ثم البنود — البنود تحتاج order_id.
       * لو فشلت البنود نحذف الترويسة حذفاً ناعماً: الحذف الحقيقي
       * للمدير وحده، وترك ترويسة بلا بنود يعني ورقة طلب فارغة.
       */
      async create(o){
        var chk = M.validateImagingOrder(o);
        if(!chk.ok) throw new Error(chk.errors[0]);
        ...
        var head = {
          ...
          clinic_id:  o.clinic_id || null,
          ordered_on: o.ordered_on,
          urgency:    o.urgency || 'routine',
          clinical_indication: M.str(o.clinical_indication) || null,
          clinical_notes:      M.str(o.clinical_notes) || null
        };
        var h = await sb().from('imaging_orders').insert(head).select('*').single();
        if(h.error) throw err(h.error, 'إنشاء الطلب');

        var items = (o.items || [])
          .filter(function(it){ return it && it.modality; })
          .map(function(it, i){
            return { order_id: h.data.id, seq: i + 1, modality: it.modality,
                     eye: it.eye, notes: M.str(it.notes) || null };
          });

        var r = await sb().from('imaging_order_items').insert(items).select('*');
        if(r.error){
          try{
            await sb().from('imaging_orders')
              .update({deleted_at: new Date().toISOString()}).eq('id', h.data.id);
          }catch(e2){}
          throw err(r.error, 'حفظ بنود الطلب');
        }
        h.data.items = r.data || items;
        return h.data;
      },

      /** التعديل يعيد كتابة البنود: طلب من ثلاث دراسات صار دراستين */
      async update(id, o){
        var patch = {};
        ['ordered_on','urgency','clinical_indication','clinical_notes',
         'visit_id','status'].forEach(function(k){
          if(o[k] !== undefined) patch[k] = (o[k] === '' ? null : o[k]);
        });
        if(Object.keys(patch).length){
          var u = await sb().from('imaging_orders').update(patch).eq('id', id);
          if(u.error) throw err(u.error, 'تعديل الطلب');
        }
        if(o.items){
          var chk = M.validateImagingOrder({
            patient_id: o.patient_id || 'x',
            ordered_on: o.ordered_on || M.today(), items: o.items });
          if(!chk.ok) throw new Error(chk.errors[0]);
          var d = await sb().from('imaging_order_items').delete().eq('order_id', id);
          if(d.error) throw err(d.error, 'تعديل بنود الطلب');
          var items = o.items.filter(function(it){ return it && it.modality; })
            .map(function(it, i){
              return { order_id: id, seq: i + 1, modality: it.modality,
                       eye: it.eye, notes: M.str(it.notes) || null };
            });
          var r = await sb().from('imaging_order_items').insert(items);
          if(r.error) throw err(r.error, 'حفظ بنود الطلب');
        }
        return imaging.orders.get(id);
      },

      async setStatus(id, status){
        if(!M.ORDER_STATUS[status]) throw new Error('حالة غير معروفة');
        var r = await sb().from('imaging_orders').update({status:status})
                .eq('id', id).select('*').single();
        if(r.error) throw err(r.error, 'تعديل حالة الطلب');
        return r.data;
      },
      ...
      async remove(id){
        var r = await sb().from('imaging_orders')
                .update({deleted_at: new Date().toISOString()})
                .eq('id', id).select('id,patient_id').single();
        if(r.error) throw err(r.error, 'حذف الطلب');
        return true;
      },

      /** ربط بند بدراسة رُفعت فعلاً — يغلق دائرة الطلب والتنفيذ */
      async fulfil(itemId, imageId){
        var r = await sb().from('imaging_order_items')
                .update({image_id: imageId || null, done: !!imageId})
                .eq('id', itemId).select('*').single();
        if(r.error) throw err(r.error, 'ربط الدراسة بالطلب');
        return r.data;
      },

      /** الطباعة تُسجَّل في القاعدة: من طبع، ومتى، وأي نسخة */
      async markPrinted(id){
        var r = await sb().rpc('mark_order_printed', {p_id:id});
        if(r.error) throw err(r.error, 'تسجيل الطباعة');
        return r.data;
      },

      async open(limit){
        var r = await sb().from('imaging_orders').select('*')
                .is('deleted_at', null).eq('status','requested')
                .order('ordered_on', {ascending:false}).limit(limit || 30);
        if(r.error) throw err(r.error, 'قراءة الطلبات المفتوحة');
        return r.data || [];
      }
    },
