...
(function (global) {
  ...
  // ══════════════════════════════════════════════════════
  // المرحلة 11 — المساعد الذكي
  //
  // ما هنا وصفٌ للعرض وتحقّقٌ لتحسين الرسائل. القرار الحقيقي في
  // القاعدة: قيد chk_rev_reject_has_reason يرفض رفضاً بلا سبب حتى
  // لو نودي بلا مرور على هذا الملف.
  // ══════════════════════════════════════════════════════

  M.AI_STATUS = {
    queued:'في الانتظار', running:'جارٍ التحليل', succeeded:'جاهز',
    failed:'فشل', superseded:'نسخة سابقة'
  };
  M.AI_STATUS_COLOR = {
    queued:'muted', running:'accent', succeeded:'success',
    failed:'danger', superseded:'muted'
  };

  M.AI_ACTION = {
    accept:'قبول', edit:'تعديل', reject:'رفض', regenerate:'إعادة توليد'
  };
  M.AI_ACTION_COLOR = {
    accept:'success', edit:'accent', reject:'danger', regenerate:'gold'
  };

  M.AI_SEVERITY = {
    none:'لا شيء', mild:'خفيف', moderate:'متوسط',
    severe:'شديد', unknown:'غير محدد'
  };
  M.AI_SEVERITY_COLOR = {
    none:'success', mild:'gold', moderate:'gold',
    severe:'danger', unknown:'muted'
  };

  M.AI_URGENCY = { routine:'عادي', soon:'قريب', urgent:'عاجل' };
  M.AI_URGENCY_COLOR = { routine:'muted', soon:'gold', urgent:'danger' };

  M.AI_QUALITY = {
    good:'جيدة', fair:'مقبولة', poor:'ضعيفة', ungradable:'غير قابلة للتقييم'
  };

  M.REPORT_STATE = { draft:'مسودة', final:'معتمد' };

  /**
   * أسماء البنود احتياطاً. المصدر الحقيقي جدول ai_finding_catalog،
   * وهذه نسخة عرض تعمل لو تأخّرت قراءة الفهرس — لا لتحلّ محلّه.
   */
  M.AI_CATEGORY_AR = {
    vitreomacular_interface:'السطح الزجاجي البقعي', foveal_contour:'تحدّب النقرة',
    cft:'سمك النقرة المركزي', retinal_layers:'طبقات الشبكية',
    irf:'سائل داخل الشبكية', srf:'سائل تحت الشبكية',
    ped:'انفصال الظهارة الصبغية', ez:'المنطقة الإهليلجية',
    rpe:'الظهارة الصبغية', choroid:'المشيمية',
    faz:'المنطقة اللاوعائية النقرية', vessel_density:'كثافة الأوعية',
    non_perfusion:'انعدام التروية', vascular_abnormalities:'شذوذات وعائية',
    neovascularization:'استحداث أوعية',
    circulation:'الدورة الدموية', arterial_filling:'الامتلاء الشرياني',
    venous_phase:'الطور الوريدي', leakage:'تسريب',
    capillary_dropout:'فقدان شعيرات',
    disc:'القرص البصري', macula:'البقعة', vessels:'الأوعية',
    hemorrhage:'نزيف', exudates:'إفرازات', edema:'وذمة',
    image_quality:'جودة الصورة', key_findings:'الموجودات الرئيسية',
    abnormality:'وجود شذوذ'
  };

  M.aiCategoryLabel = function(cat, catalog){
    if(catalog){
      for(var i=0;i<catalog.length;i++)
        if(catalog[i].category === cat) return catalog[i].label_ar;
    }
    return M.AI_CATEGORY_AR[cat] || cat || '—';
  };

  /** الفحوصات التي لها بنود في الفهرس — البقية لا يظهر لها زر تحليل */
  M.AI_MODALITIES = ['fundus','oct','octa','ffa','pentacam','visual_field',
                     'uwf_fundus','uwf_oct','uwf_octa','b_scan','other'];

  /**
   * التنبيه يظهر في كل مكان يُعرض فيه ناتج النموذج، وفي رأس التقرير
   * المُدرَج منه. تكراره مقصود: من يقرأ لوحة التحليل قد لا يكون من
   * فتحها، ومن يطبع التقرير قد لا يكون من راجعه.
   */
  M.AI_DISCLAIMER =
    'تحليل مساعد مبدئي — ليس تشخيصاً ولا يُعتمد إلا بعد مراجعة الطبيب.';
  ...
  /** نص مقروء لبند واحد — منطق العرض في مكان واحد لا في كل شاشة */
  M.aiFindingText = function(f){
    if(!f) return '—';
    var bits = [];
    if(f.value_num !== null && f.value_num !== undefined && f.value_num !== '')
      bits.push(String(f.value_num) + (f.unit ? ' ' + f.unit : ''));
    if(f.value_text) bits.push(String(f.value_text));
    if(!bits.length && f.present === true)  bits.push('موجود');
    if(!bits.length && f.present === false) bits.push('غير موجود');
    if(!bits.length) bits.push('غير مُقيَّم');
    return bits.join(' — ');
  };

  M.AI_REJECT_REASONS = ['لا يطابق الصورة','جودة الصورة لا تسمح',
    'موجودات ناقصة','موجودات خاطئة','لا يضيف شيئاً','خطأ في العين أو الفحص'];

  M.validateAiReview = function(action, o){
    var r = V(); o = o || {};
    if(!M.AI_ACTION[action]) bad(r,'قرار غير معروف');
    if(action === 'edit' && (M.str(o.text)||'').length < 3)
      bad(r,'اكتب النص المعدَّل — التعديل بلا نص ليس تعديلاً');
    if(action === 'reject' && (M.str(o.reason)||'').length < 3)
      bad(r,'اذكر سبب الرفض');
    return r;
  };

  M.validateFinalReport = function(o){
    var r = V(); o = o || {};
    if(!o.image_id)   bad(r,'الدراسة مطلوبة');
    if(!o.patient_id) bad(r,'المريض مطلوب');
    var t = M.str(o.report_text) || '';
    if(!t) bad(r,'نص التقرير فارغ');
    else if(o.finalize && t.length < 10) bad(r,'نص التقرير قصير جداً للاعتماد');
    return r;
  };
