...
Q.prototype.then=function(res, rej){
  ...
  try{
    if(this.op==='insert'){
      ...
      window.__net.writes.push({op:'insert', table:this.t, row:this.body});
      rows.push(JSON.parse(JSON.stringify(this.body)));
      out={ data:this.body, error:null };
...
const FAKE = {
  ...
  rpc: async function(name, args){ window.__net.rpc.push({name:name, args:args});
